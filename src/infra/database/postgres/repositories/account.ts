import { CheckAccountByEmail, CheckAccountRole, CreateAccount, LoadAccountByEmail, LoadAccountById, UpdateAccountById, CheckAccountById } from '@/domain/contracts/database/account'
import { Account } from '@/infra/database/postgres/entities'
import { Op } from 'sequelize'

export class AccountRepository implements CheckAccountByEmail, CreateAccount, LoadAccountByEmail, CheckAccountRole, LoadAccountById, UpdateAccountById, CheckAccountById {
  ...
  async checkById ({ id }: CheckAccountById.Input): Promise<CheckAccountById.Output> {
    const existAccount = await Account.findOne({ where: { id } })
    return existAccount !== null
  }
}
